...
trap {
    ...
    [System.Windows.Forms.MessageBox]::Show("KiRo GUI greska:" + [Environment]::NewLine + [Environment]::NewLine + $m, 'KiRo v4.1', 'OK', 'Error') | Out-Null
    exit 1
}
...
$EnginePath = Join-Path $PSScriptRoot 'KiRo_PC_Diagnostic_Repair_v4_1_ENGINE.ps1'
if (-not (Test-Path -LiteralPath $EnginePath)) {
    [System.Windows.Forms.MessageBox]::Show("Nedostaje ENGINE fajl:" + [Environment]::NewLine + $EnginePath,'KiRo v4.1') | Out-Null
    exit
}
. $EnginePath
function Pause-KiRo { }
...
function Show-KiRoMessage {
    param([string]$Text,[string]$Title='KiRo v4.1',[string]$Kind='Info')
    $icon = switch ($Kind) {
...
function Start-KiRoConsoleModule {
    ...
    try {
        ...
        Set-KiRoResult ("Otvoren modul: " + $nm + ".  Radi u zasebnom prozoru (naslov: KiRo v4.1 - ALATI: " + $Action + "). Ovaj prozor ostaje slobodan.") 'Info'
    } catch {
...
$form.Text = 'KiRo PC Diagnostic & Repair v4.1'
...
$subtitle.Text = 'v4.1   |   Skeniraj  ->  Oznaci  ->  Popravi'
